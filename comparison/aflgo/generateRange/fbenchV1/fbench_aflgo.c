...
// Values of the following variables are changed, hence passed as arguments
  ...
  compute_range(obj_dist,&range_kernel2[4]);
  compute_range(old_ray_height,&range_kernel2[5]);
  /* ------------------- END NEW ------------------- */
...
// Values of the following variables are changed, hence passed as arguments
  ...
  /*  NEW: Compute the range */
  compute_range(*iang_sin,&range_kernel3[0]);
  compute_range(from_index,&range_kernel3[1]);
  compute_range(to_index,&range_kernel3[2]);
  compute_range(slope_angle,&range_kernel3[3]);
  compute_range(rad_curvature,&range_kernel3[4]);
  /* ------------------- END NEW ------------------- */
...
void numerical_kernel4(double from_index, double to_index, 
  ...
  // Values of the following variables are changed, hence passed as arguments
    double *rang) {
   /*  NEW: Compute the range */
  compute_range(from_index,&range_kernel4[0]);
  compute_range(to_index,&range_kernel4[1]);
  compute_range(slope_angle,&range_kernel4[2]);
  compute_range(obj_dist,&range_kernel4[3]);
  /* ------------------- END NEW ------------------- */
     ...
     char *argv[];
{
  ...
  file5 = fopen("kernel5_range.dat","r");  
  ...
  if (nb_scanf != 10) {
    ...
    init_range(&range_kernel5[0]);
    init_range(&range_kernel2[0]);
    init_range(&range_kernel2[1]);
    init_range(&range_kernel3[0]);
  } else {
    range_kernel1[0].init = 1;
    range_kernel5[0].init = 1;
    range_kernel2[0].init = 1;
    range_kernel2[1].init = 1;
  ...
  for (i=0;i<4;i++) {
    ...
    if (nb_scanf != 6) {
      init_range(&range_kernel2[i+2]);
      init_range(&range_kernel3[i+1]);
      init_range(&range_kernel4[i]);
    } else {
      range_kernel2[i+2].init = 1;
      range_kernel3[i+1].init = 1;
