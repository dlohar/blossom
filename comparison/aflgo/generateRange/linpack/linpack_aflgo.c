...
/* 0-3: dx[], 4-8: dy[] */
#define NRANGE 8
__range_t range_kernel1[NRANGE];
__range_t range_kernel2[NRANGE];
...
double numerical_kernel1(double dtemp, double dx[], double dy[], int n, int ix, int iy,
  ...
  for (int j = 0; j < n; j++) {
    compute_range(dx[j],&range_kernel1[j]);
    compute_range(dy[j],&range_kernel1[j+NRANGE/2]);
  }
  /* ------------------- END NEW ------------------- */
  for (int i = 0; i < n; i++ ) {
...
double numerical_kernel2(double dtemp, double dx[], double dy[], int n) {
  ...
  /*  NEW: Check if the value is in the range. 
      If not, widen the range */
  for (int j = 0; j < n; j++) {
    compute_range(dx[j],&range_kernel2[j]);
    compute_range(dy[j],&range_kernel2[j+NRANGE/2]);
  }
  /* ------------------- END NEW ------------------- */
  for ( i = 0; i < m; i++ ) {
...
# define LDA ( N + 1 )
  ...
  /* Modification: Adding seed as input parameter */
  int init[4], nb_scanf = 0;
  /* NEW: Scan the inputs */
  nb_scanf = scanf("%lf", &cray);
  nb_scanf += scanf("%d", &init[0]);
  nb_scanf += scanf("%d", &init[1]);
  nb_scanf += scanf("%d", &init[2]);
  nb_scanf += scanf("%d", &init[3]);
  /* NEW: If the input are not the in the range, exit */
  if( (nb_scanf != 5) ||
      (cray <= 0.0) || (cray > 0.9) ||
      (init[0] < 0) || (init[0] > 10) ||
      (init[1] < 0) || (init[1] > 10) ||
      (init[2] < 0) || (init[2] > 10) ||
      (init[3] < 1000) || (init[3] > 3000) ||
      isnan(cray)) {
    exit(1);
  }
