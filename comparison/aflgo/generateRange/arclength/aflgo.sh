...
echo $'arclength_aflgo.c:10' > $TMP_DIR/BBtargets.txt
...
cd build/; $AFLGO/scripts/genDistance.sh $SUBJECT $TMP_DIR arclength_aflgo
...
cd build/; mkdir in
# ------------ Input to fuzz here ----------------------------------------
IN=`./../../../getInput/get_rand_int.sh 1 1000000`
echo $IN > in/in
touch kernel_range.dat
# ------------ Name of the executable ------------------------------------
# ----- Set timeout -> in secs (s), mins (m), hrs (h), or days (d) -------
timeout "$1"m $AFLGO/afl-fuzz -m none -z exp -c 45m -i in -o out ./arclength_aflgo
