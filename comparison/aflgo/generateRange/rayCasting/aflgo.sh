...
echo $'rayCasting_aflgo.c:27' > $TMP_DIR/BBtargets.txt
...
cd build/; $AFLGO/scripts/genDistance.sh $SUBJECT $TMP_DIR rayCasting_aflgo
...
cd build/; mkdir in
# ------------ Input to fuzz here ----------------------------------------
IN1=`./../../../getInput/get_rand_float.sh 0.0 10.0`
IN2=`./../../../getInput/get_rand_float.sh 0.0 5.0`
echo "$IN1 $IN2" > in/in
# echo "10.0 5.0" > in/in
touch kernel_range.dat
# ------------ Name of the executable ------------------------------------
# ----- Set timeout -> in secs (s), mins (m), hrs (h), or days (d) -------
timeout "$1"m $AFLGO/afl-fuzz -m none -z exp -c 45m -i in -o out ./rayCasting_aflgo
