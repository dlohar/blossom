...
// DIST computes the displacement (and its norm) between two particles.
  ...
  for (i=0;i<2;i++) {
    broaden_range(r1[i], &range_kernel1[i]);
    broaden_range(r2[i], &range_kernel1[i+2]);
  }
  /********************************/
...
void numerical_kernel2( double pos, double vel, double dt, double acc, double f,
  ...
  /* NEW: Broaden the range */
  broaden_range(pos, &range_kernel2[0]);
  broaden_range(vel, &range_kernel2[1]);
  broaden_range(dt, &range_kernel2[2]);
  broaden_range(acc, &range_kernel2[3]);
  broaden_range(f, &range_kernel2[4]);
  broaden_range(rmass, &range_kernel2[5]);
  /********************************/
...
void numerical_kernel3(double d2, double d, double f0, double f1,
  ...
  /* NEW: Broaden the range */
  broaden_range(d2, &range_kernel3[0]);
  broaden_range(d, &range_kernel3[1]);
  broaden_range(f0, &range_kernel3[2]);
  broaden_range(f1, &range_kernel3[3]);
  broaden_range(rij0, &range_kernel3[4]);
  broaden_range(rij1, &range_kernel3[5]);
  broaden_range(*pot, &range_kernel3[6]);
  /********************************/
...
/* NEW - GET RANGE: 
  ...
  FILE *old_file1, *old_file2, *old_file3;
  ...
  if ((file1 == NULL) || (old_file1 == NULL) ||
    (file2 == NULL) || (old_file2 == NULL) ||
    (file3 == NULL) || (old_file3 == NULL)){
  ...
  if (nb_scanf != 2) {
    fscanf(old_file3,"%le",&range_kernel3[6].min);
    fscanf(old_file3,"%le",&range_kernel3[6].max);
  }
  double *acc;
  double ctime;
  double e0;
