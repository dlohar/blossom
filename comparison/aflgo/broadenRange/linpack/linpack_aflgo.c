...
double numerical_kernel1(double dtemp, double dx[], double dy[], int n, int ix, int iy,
  ...
  for (int j = 0; j < n; j++) {
    broaden_range(dx[j],&range_kernel1[j]);
    broaden_range(dy[j],&range_kernel1[j+NRANGE/2]);
  }
  /* ------------------- END NEW ------------------- */
  for (int i = 0; i < n; i++ ) {
...
double numerical_kernel2(double dtemp, double dx[], double dy[], int n) {
  ...
  /*  NEW: Check if the value is in the range. 
      If not, widen the range */
  for (int j = 0; j < n; j++) {
    broaden_range(dx[j],&range_kernel2[j]);
    broaden_range(dy[j],&range_kernel2[j+NRANGE/2]);
  }
  /* ------------------- END NEW ------------------- */
  for ( i = 0; i < m; i++ ) {
