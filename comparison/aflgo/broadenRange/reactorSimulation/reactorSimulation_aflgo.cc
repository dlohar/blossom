...
double numerical_kernel1 (double u) {
  ...
  broaden_range(u, &range_kernel1[0]);
  /********************************/
  double c, value;
  double emax = 2.5;
  double emin = 1.0E-03;
...
void numerical_kernel2 (double mu, double azm, double d, double &x, double &y, double &z ) {
  /* NEW: Broaden the range */
  broaden_range(mu, &range_kernel2[0]);
  broaden_range(azm, &range_kernel2[1]);
  broaden_range(d, &range_kernel2[2]);
  broaden_range(x, &range_kernel2[3]);
  broaden_range(y, &range_kernel2[4]);
  broaden_range(z, &range_kernel2[5]);
  /********************************/
  double s;
  s = sqrt ( 1.0 - mu * mu );
  x = x + d * mu;
...
double numerical_kernel3 ( double e ) {
  /* NEW: Broaden the range */
  broaden_range(e, &range_kernel3[0]);
  /********************************/
  double s;
  double value;
  double y;
