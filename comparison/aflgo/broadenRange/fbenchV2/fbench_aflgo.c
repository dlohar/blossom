...
// Values of the following variables are changed, hence passed as arguments
  ...
  broaden_range(*iang_sin,&range_kernel1[0]);
	broaden_range(from_index,&range_kernel1[1]);
	broaden_range(to_index,&range_kernel1[2]);
	broaden_range(slope_angle,&range_kernel1[3]);
  broaden_range(obj_dist,&range_kernel1[4]);
  broaden_range(old_ray_height,&range_kernel1[5]);
...
// Values of the following variables are changed, hence passed as arguments
  ...
  /*  NEW: Check if the value is in the range. 
    ...
    broaden_range(rad_curvature,&range_kernel2[4]);
 /* ------------------- END NEW ------------------- */
...
void numerical_kernel3(double from_index, double to_index, 
  ...
  /*  NEW: Check if the value is in the range. 
      If not, widen the range */
    broaden_range(from_index,&range_kernel3[0]);
    broaden_range(to_index,&range_kernel3[1]);
    broaden_range(slope_angle,&range_kernel3[2]);
    broaden_range(obj_dist,&range_kernel3[3]);
  /* ------------------- END NEW ------------------- */
     ...
     char *argv[];
{
  ...
  FILE *file1, *file2, *file3, *old_file1, *old_file2, *old_file3;
  ...
  if (nb_scanf != 6) {
    ...
    fscanf(old_file1,"%le",&range_kernel1[1].min);
    fscanf(old_file1,"%le",&range_kernel1[1].max);
  ...
  for (i=0;i<4;i++) {
    ...
    if (nb_scanf != 6) {
      fscanf(old_file1,"%le",&range_kernel1[i+2].min);
      fscanf(old_file1,"%le",&range_kernel1[i+2].max);
      fscanf(old_file2,"%le",&range_kernel2[i+1].min);
      fscanf(old_file2,"%le",&range_kernel2[i+1].max);
