...
do
  ...
  $LLVM_DIR/build/bin/opt -load ../../src/black-box-pass/built/black_box_pass.so -blackbox -time ${runtime} -seed ${seed} -lang c++ -config ../../benchmarks/c++/$i/$i.config < busy-files/${i}.bc > busy-files/${i}_transform.bc 2>/dev/null
...
do
  ...
  $LLVM_DIR/build/bin/opt -load ../../src/black-box-pass/built/black_box_pass.so -blackbox -time ${runtime} -seed ${seed} -lang rust -config ../../benchmarks/rust/$i/$i.config < busy-files/${i}.bc > busy-files/${i}_transform.bc 2>/dev/null
  $LLVM_DIR/build/bin/llc -filetype=obj busy-files/${i}_transform.bc
