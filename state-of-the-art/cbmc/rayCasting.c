...
vec numerical_kernel(vec a, double s, vec b)
{
  ...
  c.x = a.x + s * b.x;
  c.y = a.y + s * b.y;
  __CPROVER_assert(!(isinf(c.x)), "infinity");
  __CPROVER_assert(!(isnan(c.x)), "nan");
  __CPROVER_assert(!(isinf(c.y)), "infinity");
  __CPROVER_assert(!(isnan(c.y)), "nan");
  return c;
}
...
{
  double c1, c2;
  __CPROVER_assume((c1 >= 0.0) && (c1 <= 10.0));
  __CPROVER_assume((c2 >= 0.0) && (c2 <= 5.0));
