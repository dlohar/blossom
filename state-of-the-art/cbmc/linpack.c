...
double numerical_kernel1(double dtemp, double dx[], double dy[], int n, int ix, int iy,
  ...
  __CPROVER_assert(!(isinf(dtemp)), "infinity");
  __CPROVER_assert(!(isnan(dtemp)), "nan");
  return dtemp;
}
...
# define LDA ( N + 1 )
  ...
  double b_max;
  double cray;
  __CPROVER_assume((cray > 0.0) && (cray <= 0.9));
  double eps;
  int i;
  int info;
  int *ipvt;
  ...
  double *x;
  int init[4];
  __CPROVER_assume((init[0] >= 0) && (init[0] <= 10));
  __CPROVER_assume((init[1] >= 0) && (init[1] <= 10));
  __CPROVER_assume((init[2] >= 0) && (init[2] <= 10));
  __CPROVER_assume((init[3] >= 1000) && (init[3] <= 3000));
  timestamp ( );
