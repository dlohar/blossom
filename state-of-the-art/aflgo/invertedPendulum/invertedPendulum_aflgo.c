...
double numerical_kernel1(double in[4], double constraint) {
  ...
  if (w == 0.0){
    assert(0 && "Division by zero in the kernel 1");
  }
  ...
  if (isnan(w) || isnan(E) || isnan(a) || isnan(F) || isnan(V)){
    assert(0 && "NaN in the kernel 1");
  }
  if (isinf(w) || isinf(E) || isinf(a) || isinf(F) || isinf(V)){
...
void numerical_kernel2(double in[4], double V, double out[4]) {
  ...
  if ((div1 == 0.0) || (div2 == 0.0)){
    assert(0 && "Division by zero in the kernel 2");
  }
  ...
  /* Check for NaNs and infinity */
  if (isnan(x2_dt) || isnan(x4_dt)){
    assert(0 && "NaN in the kernel 2");
  }
  if (isinf(x2_dt) || isinf(x4_dt)){
