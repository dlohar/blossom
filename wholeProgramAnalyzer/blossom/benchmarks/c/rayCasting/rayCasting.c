...
/* vmadd function */
vec numerical_kernel(double x1, double y1, double s, double x2, double y2) 
{
  vec c;
  c.x = x1 + s * x2;
  c.y = y1 + s * y2;
...
int intersect(vec x0, vec x1, vec y0, vec y1, double tol, vec *sect)
{
  ...
  if (sect)
    *sect = numerical_kernel(y0.x, y0.y, a, dy.x, dy.y);
...
void original_program_main(double c1, double c2)
{
